  ...
  public:
    ...
    bool heapProperty(struct Node* root){
        ...
        if(root->left && !root->right){
            ...
            return root->data > root->left->data &&  root->data > root->right->data && heapProperty(root->left) && heapProperty(root->right);
        }
    }
    
    bool isHeap(struct Node* tree) {
        int totcnt = cntNode(tree);
        return isCBT(tree,0,totcnt) && heapProperty(tree); 
